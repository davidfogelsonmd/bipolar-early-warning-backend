...
async function fetchAndUpdatePatient(patientId,accessToken,patients){
  ...
  // Sleep and activity are required; these have always been accessible.
  ...
  // Readiness (temperature) is OPTIONAL: older tokens may lack readiness scope and return 401.
  // Wrap it so a failure here NEVER breaks the whole refresh — temperature just stays null.
  var readinessData=[];
  try {
    var rRes=await axios.get('https://api.ouraring.com/v2/usercollection/daily_readiness?start_date='+start+'&end_date='+end,{headers:headers});
    readinessData=rRes.data.data;
  } catch(rErr){
    console.log('Readiness unavailable for '+patientId+' (status '+(rErr.response?rErr.response.status:rErr.message)+') — temperature will be null until re-enrolled');
    readinessData=[];
  }
  var days=[];
  for(var i=6;i>=0;i--){var d=new Date();d.setDate(d.getDate()-i);days.push(d.toISOString().split('T')[0]);}
  var parsed=parseOuraData(results[0].data.data,results[1].data.data,readinessData,days);
